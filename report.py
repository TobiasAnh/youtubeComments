...
videos = pd.read_csv(processed_path.joinpath("videos.csv"), lineterminator="\r", index_col="videoId")
...
comments = pd.read_csv(processed_path.joinpath("comments.csv"), lineterminator="\r", index_col=0)
...
monthly_comments = (
    comments.query("comment_published < @report_deadline")
    .groupby(pd.Grouper(key="comment_published", freq=frequency))
    .size()
)

timeseries_comments = px.line(monthly_comments, template="simple_white")
...
for channel in list(channels.channelTitle):
    plt.clf()
    comment_authors_channel = comments.query("videoOwnerChannelTitle == @channel")["comment_author"].unique()
    comment_authors_AllOtherChannels = comments.query("videoOwnerChannelTitle != @channel")["comment_author"].unique()
    authors_outside_activity = set(comment_authors_channel).intersection(set(comment_authors_AllOtherChannels))
    n_authors_outside_activity = len(authors_outside_activity)
    fraction = len(authors_outside_activity) / len(comment_authors_channel)

    # Create sets for the Venn diagram
    set1 = set(comment_authors_channel)
    set2 = set(comment_authors_AllOtherChannels)

    # Create the Venn diagram object
    venn = venn2([set1, set2], set_labels=(f"{channel}", "Andere ZDF channels"))

    # Customize the Venn diagram
    venn.get_label_by_id("10").set_text(len(set1))
    venn.get_label_by_id("01").set_text(len(set2))
    venn.get_label_by_id("11").set_text(n_authors_outside_activity)

    # Display the Venn diagram
    plt.title(f"Outside channel activity {channel} Autor*innen: {round(fraction*100, 1)}%")
    plt.suptitle(f"Anzahl Autor*innen von {channel} vs. ZDF gesamt")
    path = reports_path.joinpath(channel.replace("&", "_"))
